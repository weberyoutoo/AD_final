...
import numpy as np
from os.path import expanduser, join, exists, basename, split
...
from anomalib.data import MVTecLoco, MVTec
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--epochs_per_cat", type=int,
...
def run(module_path: str, class_name: str, weights_path: str, dataset_path: str, category: str, ckpt_dir: str, epochs_per_cat: int) -> None:
    ...
    categorys = [basename(f) for f in (
        glob(join("./datasets/mvtec_anomaly_detection", "*")))]
    ...
    for i in range(1, 51):
        ...
        ad_datamodule = MVTec(root=ad_dataset_path,
                              category=categorys[randIdx],
                              train_batch_size=1,
                              image_size=(256, 256))
        ad_datamodule.setup()
        print(
            f"======================Start training with MV-tec category : {ad_datamodule.category}======================")
        # set 1 for experiment
        model.fit(datamodule=ad_datamodule,
                  cur_epoch=i * epochs_per_cat,
                  ckpt_dir=ckpt_dir,
                  max_epochs=epochs_per_cat)
    #
    # Create the dataset
...
if __name__ == "__main__":
    ...
    run(args.module_path, args.class_name,
        args.weights_path, args.dataset_path,
        args.category, args.ckpt_dir,
        args.epochs_per_cat)
